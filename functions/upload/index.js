...
export async function onRequestGet(context) {
  ...
  return new Response(
    ...
    <div class="card">
      ...
      <div id="voorNaUploader" style="display:none">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
          <div>
            <div style="font-size:12px;font-weight:600;margin-bottom:6px;text-align:center">Voor</div>
            <div class="dropzone" id="voorDropzone" style="padding:16px 8px;margin-bottom:0">
              <img id="voorPreview" style="display:none;width:100%;aspect-ratio:1;object-fit:cover;border-radius:6px">
              <span id="voorLabel">Kies voor-foto</span>
              <input type="file" id="voorInput" accept="image/*">
            </div>
          </div>
          <div>
            <div style="font-size:12px;font-weight:600;margin-bottom:6px;text-align:center">Na</div>
            <div class="dropzone" id="naDropzone" style="padding:16px 8px;margin-bottom:0">
              <img id="naPreview" style="display:none;width:100%;aspect-ratio:1;object-fit:cover;border-radius:6px">
              <span id="naLabel">Kies na-foto</span>
              <input type="file" id="naInput" accept="image/*">
            </div>
          </div>
        </div>
        <button class="primary" id="voorNaUploadBtn" disabled>Paar toevoegen</button>
      </div>
    ...
    <script>
      ...
      const normalUploader = document.getElementById('normalUploader');
      const voorNaUploader = document.getElementById('voorNaUploader');
      ...
      const voorInput = document.getElementById('voorInput');
      const naInput = document.getElementById('naInput');
      const voorPreview = document.getElementById('voorPreview');
      const naPreview = document.getElementById('naPreview');
      const voorLabel = document.getElementById('voorLabel');
      const naLabel = document.getElementById('naLabel');
      const voorNaUploadBtn = document.getElementById('voorNaUploadBtn');
      ...
      tabs.forEach((tab) => {
        tab.addEventListener('click', () => {
          ...
          resetPicker();
          resetVoorNaPicker();
          if (currentCategory === 'voor-na') {
            normalUploader.style.display = 'none';
            voorNaUploader.style.display = 'block';
          } else {
            normalUploader.style.display = 'block';
            voorNaUploader.style.display = 'none';
          }
          loadGallery();
        });
      });

      // Normale uploader (buiten/binnen/portfolio)
      ...
      // Voor & Na uploader (paar)
      document.getElementById('voorDropzone').addEventListener('click', () => voorInput.click());
      document.getElementById('naDropzone').addEventListener('click', () => naInput.click());

      voorInput.addEventListener('change', () => {
        if (!voorInput.files[0]) return;
        voorFile = voorInput.files[0];
        voorPreview.src = URL.createObjectURL(voorFile);
        voorPreview.style.display = 'block';
        voorLabel.style.display = 'none';
        voorNaUploadBtn.disabled = !(voorFile && naFile);
      });
      naInput.addEventListener('change', () => {
        if (!naInput.files[0]) return;
        naFile = naInput.files[0];
        naPreview.src = URL.createObjectURL(naFile);
        naPreview.style.display = 'block';
        naLabel.style.display = 'none';
        voorNaUploadBtn.disabled = !(voorFile && naFile);
      });

      function resetVoorNaPicker() {
        voorFile = null;
        naFile = null;
        voorInput.value = '';
        naInput.value = '';
        voorPreview.style.display = 'none';
        naPreview.style.display = 'none';
        voorLabel.style.display = 'inline';
        naLabel.style.display = 'inline';
        voorNaUploadBtn.disabled = true;
        msg.textContent = '';
      }

      voorNaUploadBtn.addEventListener('click', async () => {
        ...
        const fd = new FormData();
        fd.append('category', 'voor-na');
        fd.append('voorPhoto', voorFile);
        fd.append('naPhoto', naFile);
        ...
        try {
          ...
          const data = await res.json();
          if (res.ok) {
            msg.textContent = 'Paar succesvol toegevoegd!';
            msg.className = 'msg success';
            resetVoorNaPicker();
            loadGallery();
          } else {
        ...
        progress.textContent = '';
        voorNaUploadBtn.disabled = false;
      });
      ...
      async function loadGallery() {
        ...
        try {
          ...
          const data = await res.json();
          if (currentCategory === 'voor-na') {
            renderVoorNaGallery(data.photos || []);
          } else {
      ...
      function renderVoorNaGallery(pairs) {
        if (!pairs.length) {
          gallery.innerHTML = '<div class="gallery-empty">Nog geen paren in deze categorie.</div>';
          return;
        }
        gallery.innerHTML = '';
        pairs.forEach((p, i) => {
          ...
          row.className = 'gallery-item';
          row.innerHTML =
            '<img src="' + p.voorUrl + '" loading="lazy" style="border-radius:6px 0 0 6px">' +
            '<img src="' + p.naUrl + '" loading="lazy" style="border-radius:0 6px 6px 0;margin-left:-6px">' +
            '<div class="name">Paar ' + (i + 1) + '</div>' +
            '<div class="actions">' +
            '<button class="icon-btn up" ' + (i === 0 ? 'disabled' : '') + '>↑</button>' +
            '<button class="icon-btn down" ' + (i === pairs.length - 1 ? 'disabled' : '') + '>↓</button>' +
            '<button class="icon-btn danger del">✕</button>' +
            '</div>';
          row.querySelector('.up').addEventListener('click', () => reorder(p.pairId, 'up'));
          row.querySelector('.down').addEventListener('click', () => reorder(p.pairId, 'down'));
          row.querySelector('.del').addEventListener('click', () => deletePhoto(p.pairId));
          gallery.appendChild(row);
        });
      }
