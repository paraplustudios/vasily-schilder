...
export async function onRequestPost(context) {
  ...
  if (category === 'voor-na') {
    const voorFile = formData.get('voorPhoto');
    const naFile = formData.get('naPhoto');
    if (!voorFile || !naFile) {
      return new Response(JSON.stringify({ error: "Voor- en na-foto zijn beide verplicht" }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    const pairId = `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
    await bucket.put(`voor-na/${pairId}-voor.jpg`, voorFile.stream(), {
      httpMetadata: { contentType: voorFile.type || 'image/jpeg' },
    });
    await bucket.put(`voor-na/${pairId}-na.jpg`, naFile.stream(), {
      httpMetadata: { contentType: naFile.type || 'image/jpeg' },
    });
